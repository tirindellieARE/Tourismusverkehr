...
# AT merged into "other" (same as model 2)
...
# --- Selected attractivity indexes only (log1p + z-standardised) ---
attr_cols <- c(
...
topo_betas <- setNames(rep(0, 6),
                       c(paste0("beta_topo2_", nats), paste0("beta_topo3_", nats)))
attr_short <- c("v01", "v02", "v03", "v07", "v09", "v12")
attr_betas <- setNames(
...
apollo_probabilities <- function(apollo_beta, apollo_inputs, functionality = "estimate") {
  ...
  for (j in 1:N_ALTS) {
    ...
    v <- beta_tt_DE * tt_j * (nat_group == "DE") +
         ...
         beta_v01_DE * get(paste0(attr_cols[1], "_", j)) * (nat_group == "DE") +
         beta_v01_FR * get(paste0(attr_cols[1], "_", j)) * (nat_group == "FR") +
         beta_v01_IT * get(paste0(attr_cols[1], "_", j)) * (nat_group == "IT") +
         beta_v02_DE * get(paste0(attr_cols[2], "_", j)) * (nat_group == "DE") +
         beta_v02_FR * get(paste0(attr_cols[2], "_", j)) * (nat_group == "FR") +
         beta_v02_IT * get(paste0(attr_cols[2], "_", j)) * (nat_group == "IT") +
         beta_v03_DE * get(paste0(attr_cols[3], "_", j)) * (nat_group == "DE") +
         beta_v03_FR * get(paste0(attr_cols[3], "_", j)) * (nat_group == "FR") +
         beta_v03_IT * get(paste0(attr_cols[3], "_", j)) * (nat_group == "IT") +
         beta_v07_DE * get(paste0(attr_cols[4], "_", j)) * (nat_group == "DE") +
         beta_v07_FR * get(paste0(attr_cols[4], "_", j)) * (nat_group == "FR") +
         beta_v07_IT * get(paste0(attr_cols[4], "_", j)) * (nat_group == "IT") +
         beta_v09_DE * get(paste0(attr_cols[5], "_", j)) * (nat_group == "DE") +
         beta_v09_FR * get(paste0(attr_cols[5], "_", j)) * (nat_group == "FR") +
         beta_v09_IT * get(paste0(attr_cols[5], "_", j)) * (nat_group == "IT") +
         beta_v12_DE * get(paste0(attr_cols[6], "_", j)) * (nat_group == "DE") +
         beta_v12_FR * get(paste0(attr_cols[6], "_", j)) * (nat_group == "FR") +
         beta_v12_IT * get(paste0(attr_cols[6], "_", j)) * (nat_group == "IT")
...
fwrite(est, "results_output/attr_minimal_10k_results.csv")
